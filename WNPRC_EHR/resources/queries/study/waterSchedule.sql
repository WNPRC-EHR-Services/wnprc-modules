...
null as restraint,
null as time,
(SELECT max(d.qcstate) as label FROM study.water_given d WHERE (s.objectid || '||' || s.date) = d.parentid) as treatmentStatus,
(SELECT max(taskId) as taskId FROM study.water_given d WHERE (s.objectid || '||' || s.date) = d.parentid) as taskId
...
END as volume2,
TRUNCATE(round(CAST(t1.volume AS NUMERIC),4),4) as volume,
t1.vol_units,
case
...
CASE
  ...
  WHEN (t1.frequency.meaning='Daily - Any Time')
    THEN 'Any Time'
  WHEN (t1.frequency.meaning='Daily - PM' OR t1.frequency.meaning='Weekly - PM' OR t1.frequency.meaning='Alternating Days - PM')
    THEN 'PM'
  WHEN (t1.frequency.meaning='Daily - Night')
    THEN 'Night'
END as TimeOfDay,
...
LEFT JOIN study."Treatment Orders" t1
  ON (t1.code = 'c-10120' AND d.date >= t1.date and (d.date <= cast(t1.enddate as date) OR t1.enddate is null) AND (
  --daily
